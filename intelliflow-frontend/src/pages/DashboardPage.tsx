...
export const DashboardPage: React.FC = () => {
  ...
  const metrics = [
    { title: 'Task Orchestration', value: 'Active Tasks', change: 'Sprint Backlog & Kanban', icon: CheckSquare },
    { title: 'Ingested Documents', value: 'Knowledge Base', change: '384-dim dense vectors', icon: FileText },
    { title: 'Vector Search SLA', value: 'Sub-Millisecond', change: 'HNSW pgvector index', icon: Search },
    { title: 'RAG Pipeline Status', value: 'Grounded Engine', change: 'FastAPI + Groq LPU', icon: Cpu },
  ];
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between p-6 rounded-2xl glass-card-gold border border-amber-500/30">
        ...
        <div className="mt-4 md:mt-0 flex items-center space-x-3">
          <button
            ...
            className="px-4 py-2 rounded-xl text-xs font-semibold glass-card border border-white/10 text-zinc-200 hover:border-amber-500/40 transition-colors flex items-center space-x-2 cursor-pointer"
          >
          ...
          <button
            ...
            className="px-4 py-2 rounded-xl text-xs font-bold gold-gradient-bg text-zinc-950 shadow-gold-sm hover:brightness-110 transition-all flex items-center space-x-2 cursor-pointer"
          >
        ...
        {metrics.map((metric, idx) => {
          ...
          return (
            <div key={idx} className="p-5 rounded-2xl glass-card border border-white/10 hover:border-amber-500/30 transition-all group">
              ...
              <div className="mt-3 text-lg font-extrabold text-zinc-100">{metric.value}</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 p-6 rounded-2xl glass-card border border-white/10 space-y-4">
          <div className="flex items-center justify-between border-b border-white/10 pb-4">
            ...
            <button onClick={() => navigate('/tasks')} className="text-xs text-amber-400 cursor-pointer hover:underline bg-transparent border-none">
              View All Tasks →
            </button>
          </div>
          ...
          <div className="space-y-4">
            <div>
              <div className="flex justify-between text-xs text-zinc-300 mb-1">
                ...
                <span className="text-amber-400 font-bold font-mono">sentence-transformers</span>
              </div>
              <div className="w-full h-2 bg-zinc-800 rounded-full overflow-hidden">
                <div className="h-full bg-amber-400 rounded-full w-[100%]"></div>
              </div>
            </div>
            ...
            <div>
              <div className="flex justify-between text-xs text-zinc-300 mb-1">
                ...
                <span className="text-amber-400 font-bold font-mono">384-dimensional</span>
              </div>
              <div className="w-full h-2 bg-zinc-800 rounded-full overflow-hidden">
                <div className="h-full bg-amber-400 rounded-full w-[100%]"></div>
              </div>
            ...
            <div className="p-4 rounded-xl bg-amber-500/10 border border-amber-500/20 text-xs text-amber-300">
              ...
              <p className="text-[11px] text-amber-400/80">384-dimensional embeddings stored with native vector_cosine_ops index.</p>
            </div>
          </div>
        </div>
      </div>
